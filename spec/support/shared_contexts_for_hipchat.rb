...
shared_context 'HipChatV1' do
  ...
  def mock_successful_topic_change(topic, options={})
    ...
    stub_request(:post, 'https://api.hipchat.com/v1/rooms/topic')
        .with( query:   { auth_token: 'blah' },
               body:    { room_id:    'Hipchat',
                          from:       options[:from],
                          topic:      'Nice topic' },
  ...
  def mock_successful_rooms
    stub_request(:get, 'https://api.hipchat.com/v1/rooms/list')
        .with( query:   { auth_token: 'blah' },
               body:    '',
               headers: { 'Accept':       'application/json',
                          'Content-Type': 'application/x-www-form-urlencoded' })
...
shared_context 'HipChatV2' do
  ...
  def mock_successful_room_creation(name, options = {})
    stub_request(:post, 'https://api.hipchat.com/v2/room')
      .with(query:    { auth_token: 'blah' },
  ...
  def mock_successful_invite(user_id, options={})
    options = {:user_id => user_id }.merge(options)
  ...
  def mock_successful_add_member(user_id, options={})
    options = {:user_id => user_id }.merge(options)
